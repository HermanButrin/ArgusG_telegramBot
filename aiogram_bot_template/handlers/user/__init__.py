...
from . import ban_handler, coin_handler, item_handler, menu_handler, profile_handler, promote_handler, shop_handler, unban_handler, musician_handler
...
def prepare_router() -> Router:
    ...
    user_router.message.register(promote_handler.promote, Command("promote"), AdminFilter())
    user_router.callback_query.register(
        musician_handler.musician,
        Action.filter(),
        lambda _callback, callback_data: getattr(callback_data, "action", None) == "musician",
    )
    user_router.callback_query.register(
        musician_handler.inventory,
        Action.filter(),
        lambda _callback, callback_data: getattr(callback_data, "action", None) == "inventory",
    )
    user_router.callback_query.register(
